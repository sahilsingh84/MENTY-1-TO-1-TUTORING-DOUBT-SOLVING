import React from "react";
import { JackInTheBox } from 'react-awesome-reveal';

const Stats = [
  { count: "5K", label: "Active Students" },
  { count: "10+", label: "Mentors" },
  { count: "200+", label: "Courses" },
  { count: "50+", label: "Awards" },
];

const StatsComponenet = () => {
  return (
    
    <div className="bg-richblack-700 subCustomBoxShadow overflow-y-none">
      <div className="flex flex-col gap-10 justify-between w-11/12 max-w-maxContent text-black mx-auto ">

        <div className="grid grid-cols-2 md:grid-cols-4 text-center">
              { Stats.map((data, index) => {
                  return (
                  <JackInTheBox>
                    <div className="flex flex-col py-10 overflow-hidden" key={index} >
                      <h1 className="text-[30px] font-bold text-[#EC1D1D]"> {data.count} </h1>
                      <h2 className="font-semibold text-[16px] text-richblack-500"> {data.label} </h2>   
                    </div>
                    </JackInTheBox>
                    )})
               }
        </div>

      </div>
    </div>
 
)};

export default StatsComponenet;